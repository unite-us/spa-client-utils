...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Choices = function () {
	  function Choices() {
	    ...
	    var ie11Agent = navigator.userAgent.match(/rv[ :]11/);
	    var ie10Agent = navigator.userAgent.match(/MSIE\s10/);
	    ...
	    this.isOldIE = !!(navigator.userAgent.match(/Trident/) && (ie11Agent || ie10Agent));
	  ...
	  _createClass(Choices, [{
	        ...
	        if (this.currentState.choices !== this.prevState.choices || this.currentState.groups !== this.prevState.groups || this.currentState.items !== this.prevState.items) {
	          if (this.isSelectElement) {
	            ...
	            if (activeGroups.length >= 1 && this.isSearching !== true) {
	              activeChoices.reduce(function (groupIds, activeChoice) {
	                var groupId = activeChoice.groupId;
	                // If the choice is not part of a group
	                if (groupId < 0 || groupId === undefined) {
	                  choiceListFragment = _this4.renderChoices([activeChoice], choiceListFragment);
	                } else {
	                  // If the choice group hasn't already been rendered
	                  if (groupIds.indexOf(groupId) < 0) {
	                    // Get the group corresponding to the choice
	                    var activeGroup = activeGroups.filter(function (group) {
	                      return group.id === groupId;
	                    });
	                    choiceListFragment = _this4.renderGroups(activeGroup, activeChoices, choiceListFragment);
	        ...
	        choiceValue.forEach(function (val) {
	          ...
	          if (foundChoice) {
	            if (!foundChoice.selected) {
	              _this12._addItem(foundChoice.value, foundChoice.label, foundChoice.id, foundChoice.groupId, foundChoice.customProperties, foundChoice.placeholder, foundChoice.keyCode);
	      ...
	      return function (results, value, label) {
	        ...
	        if (parsedResults && (0, _utils.isType)('Array', parsedResults) && parsedResults.length) {
	          ...
	          parsedResults.forEach(function (result) {
	            if (result.choices) {
	              var _groupId = result.id || null;
	              _this15._addGroup(result, _groupId, value, label);
	            } else {
	    ...
	    value: function _handleSearch(value) {
	      if (!value) {
	        ...
	        (0, _utils.triggerEvent)(this.passedElement, 'search', { value: value, resultCount: 0 });
	        return;
	      }
	    ...
	    value: function _onMouseDown(e) {
	      ...
	      // If we have our mouse down on the scrollbar and are on IE11 or IE10...
	      if (target === this.choiceList && this.isOldIE) {
	    ...
	    value: function _onBlur(e) {
	      ...
	      if (this.containerOuter.contains(target) && !this.isScrollingOnIe) {
	        ...
	        // On IE11 and IE10, clicking the scrollbar blurs our input and thus
	        // closes the dropdown. To stop this, we refocus our input
	        // if we know we are on IE *and* are scrolling.
	        // See _onMouseDown for logic on setting true.
	        this.isScrollingOnIe = false;
	        this.input.focus();
	      }
	    }
...
/***/ (function(module, exports) {
	...
	var items = function items() {
	  ...
	  switch (action.type) {
	    ...
	    case 'ADD_ALL_ITEMS':
	      {
	        return [].concat(_toConsumableArray(state), [action.allItems]);
	      }
...
/***/ (function(module, exports) {
	...
	var groups = function groups() {
	  ...
	  switch (action.type) {
	    ...
	    case 'ADD_ALL_GROUPS':
	      {
	        return [].concat(_toConsumableArray(state), _toConsumableArray(action.allGroups));
	      }
	...
	var choices = function choices() {
	  ...
	  switch (action.type) {
	    ...
	    case 'ADD_ALL_CHOICES':
	      {
	        return [].concat(_toConsumableArray(state), _toConsumableArray(action.allChoices));
	      }
...
/***/ (function(module, exports) {
	...
	var addAllChoices = exports.addAllChoices = function addAllChoices(allChoices) {
	  return {
	...
	var addAllGroups = exports.addAllGroups = function addAllGroups(allGroups) {
	  return {
	    type: 'ADD_ALL_GROUPS',
	    allGroups: allGroups
	  };
	...
	var addAllItems = exports.addAllItems = function addAllItems(allItems) {
	  return {
	    type: 'ADD_ALL_ITEMS',
	    allItems: allItems
	  };
